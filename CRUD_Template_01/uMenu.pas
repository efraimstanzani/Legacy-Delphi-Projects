...
type
  TfrmMenu = class(TForm)
    ...
    procedure dbedt_EstoqKeyPress(Sender: TObject; var Key: Char);
...
begin
  if (dbedt_Estoq.Text <> '') and (StrToInt(dbedt_Estoq.Text) >= 0) then
    dbedt_Estoq.Text := IntToStr(Succ(StrToInt(dbedt_Estoq.Text)));
end;

procedure TfrmMenu.btn_MenosClick(Sender: TObject);
begin
  if (dbedt_Estoq.Text <> '') and (StrToInt(dbedt_Estoq.Text) > 0) then
    dbedt_Estoq.Text := IntToStr(Pred(StrToInt(dbedt_Estoq.Text)));
end;
...
procedure TfrmMenu.dbedt_EstoqKeyPress(Sender: TObject; var Key: Char);
begin
  //Aceitando apenas números
  if ((key in ['0'..'9'] = false) and (word(key) <> vk_back)) then
 key := #0;
...
procedure TfrmMenu.btn_SalvarClick(Sender: TObject);
begin

  qry_Prods.Edit;
  qry_Prods.FieldByName('ID_PRODUT').AsString := dbedt_Id.Text;
  qry_Prods.FieldByName('NOME_PRODU').AsString := dbedt_Nome.Text;
  qry_Prods.FieldByName('ID_CATEGORIA').AsString := dblkcbb_Categoria.Text;
  qry_Prods.FieldByName('QTDE_PRODUTO').AsString := dbedt_Estoq.Text;
  qry_Prods.Post;
  qry_Prods.Close;
